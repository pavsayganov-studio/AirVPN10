#!/bin/bash
# =============================================================================
# Raketa v0.9.3 — HIG compliance + pip fix
# Run from repo root: bash patch_raketa_093.sh
# =============================================================================
set -e
G='\033[0;32m'; Y='\033[1;33m'; C='\033[0;36m'; R='\033[0;31m'; N='\033[0m'

echo -e "${C}╔══════════════════════════════════════════╗"
echo -e "║   Raketa v0.9.3 — HIG + pip fix          ║"
echo -e "╚══════════════════════════════════════════╝${N}\n"

[ ! -f "ViewController.m" ] && echo -e "${R}✗ Run from repo root${N}" && exit 1

cp ViewController.m ViewController.m.bak093
cp Info.plist Info.plist.bak093
cp .github/workflows/build.yml .github/workflows/build.yml.bak093
echo -e "${G}✓ Backups created${N}\n"

# =============================================================================
# 1. ViewController.m — full rewrite with HIG spacing & typography
#
# HIG 10.13 changes applied (zero CPU cost — layout/font only):
#   kPAD 14→20  (HIG: 20pt outer margin)
#   kH 296→314  (extra room from correct padding)
#   Header 44→32pt
#   Section labels 9→11pt  (HIG: 11pt Small)
#   Dropdown/base text 12→13pt  (HIG: 13pt Regular)
#   Bottom buttons 10→11pt  (HIG: 11pt Small)
#   Connect button corner radius = height/2 (true capsule feel)
#   Between-group spacing 12pt  (HIG: 12-16pt)
#   Related-element spacing 8pt  (HIG: 8pt)
# =============================================================================
...
# =============================================================================
# 3. build.yml — fix pip + keep icon pipeline
# =============================================================================
echo -e "${Y}→ build.yml (v0.9.3)${N}"
...
jobs:
  build:
    ...
    - name: Generate AppIcon.icns from SVG
      run: |
        # --break-system-packages: required on macOS runners with PEP 668
    ...
    - name: Release
      ...
      with:
        ...
        body: |
          ...
          ### v0.9.3 — Production release
          **Исправлено**
          - pip `--break-system-packages` — иконка теперь собирается без ошибки PEP 668

          **HIG 10.13 compliance (zero CPU cost)**
          - Внешние отступы 20pt (был 14pt)
          - Шрифты: 13pt базовый, 11pt вторичный, 9pt микро — точно по HIG
          - Высота кнопок: 21pt для Логи/Выход, connect = 36pt capsule (corner 18)
          - Межгрупповые отступы 12pt, связанные элементы 8pt
          - Header уменьшен до 32pt (ближе к HIG 22pt title bar)
          - Все предыдущие исправления v0.9.x включены
...
echo ""
echo -e "${C}╔══════════════════════════════════════════════════════╗"
echo -e "║  Patch applied. Ready for production:                ║"
echo -e "╠══════════════════════════════════════════════════════╣"
echo -e "║  git add -A                                          ║"
echo -e "║  git commit -m 'v0.9.3: HIG polish + pip fix'       ║"
echo -e "║  git tag v0.9.3                                      ║"
