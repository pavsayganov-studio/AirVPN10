#!/bin/bash
# =============================================================================
# PauloVPN — Telegram Fix Patch
# Запускать из корня репозитория в GitHub Codespaces:
#   bash patch_paulovpn.sh
# =============================================================================
...
echo -e "${CYAN}╔══════════════════════════════════════════╗"
echo -e "║   PauloVPN — Telegram MTProxy Patch      ║"
echo -e "╚══════════════════════════════════════════╝${NC}\n"

# Проверяем что мы в корне репозитория
if [ ! -f "ViewController.m" ]; then
    echo -e "${RED}✗ Запусти скрипт из корня репозитория (где лежит ViewController.m)${NC}"
    exit 1
fi
...
# Бэкап
cp ViewController.m ViewController.m.bak
echo -e "${GREEN}✓ Бэкап создан: ViewController.m.bak${NC}\n"

# =============================================================================
# ПИШЕМ НОВЫЙ ViewController.m полностью
# =============================================================================

echo -e "${YELLOW}→ Записываю обновлённый ViewController.m...${NC}"
...
echo -e "${GREEN}✓ ViewController.m обновлён${NC}"

# =============================================================================
# Обновляем Info.plist — CFBundleVersion + правильный executable name
# =============================================================================
echo -e "${YELLOW}→ Обновляю Info.plist...${NC}"
...
<dict>
    ...
    <string>AirVPN</string>
    <key>CFBundleIdentifier</key>
    <string>com.samurai.paulovpn</string>
    <key>CFBundleName</key>
    <string>PauloVPN</string>
    ...
    <string>PauloVPN v0.6</string>
</dict>
</plist>
PLIST_EOF

echo -e "${GREEN}✓ Info.plist обновлён (v0.6.0)${NC}"

# =============================================================================
# Обновляем build.yml — добавляем SystemConfiguration framework
# =============================================================================
echo -e "${YELLOW}→ Обновляю build.yml...${NC}"
...
jobs:
  build:
    ...
    - name: Build sing-box core
      run: |
        ...
        CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 \
          go build \
            -tags "with_utls,with_grpc,with_reality" \
            -trimpath \
            -ldflags="-s -w" \
            -o ../sing-box \
            ./cmd/sing-box
    ...
    - name: Compile macOS app
      run: |
        mkdir -p AirVPN.app/Contents/MacOS
        mkdir -p AirVPN.app/Contents/Resources
        ...
        # Компилируем с SystemConfiguration (нужен для SCDynamicStoreCopyProxies)
        clang \
          -fobjc-arc \
          -framework Cocoa \
        ...
        zip -r "PauloVPN-macOS-10.13-${{ github.ref_name }}.zip" AirVPN.app

    - name: Create Release
      uses: softprops/action-gh-release@v1
      with:
        tag_name: ${{ github.ref_name }}
        name: "PauloVPN ${{ github.ref_name }}"
        body: |
          ## PauloVPN ${{ github.ref_name }}

          ### Что нового в v0.6
          - ✅ Telegram: дополнительный SOCKS5 на порту 10810 + deep link одним кликом
          - ✅ Watchdog: автообнаружение падения ядра, сброс прокси без пароля
          - ✅ Панель настройки Telegram прямо в интерфейсе
          - ✅ Убраны selector/urltest группы из конфига — нет фоновых пингов
          - ✅ Telegram IP больше не в direct — Telegram работает через VPN
          - ✅ Shell injection protection — эскейпинг путей
          - ✅ Лог перезаписывается при старте (не растёт бесконечно)
          - ✅ Пароль только когда реально нужен

          ### Telegram
          Нажми кнопку "📱 Настройка Telegram" в приложении.
        files: PauloVPN-macOS-10.13-*.zip
      env:
        GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
YAML_EOF

echo -e "${GREEN}✓ build.yml обновлён${NC}"
...
echo ""
echo -e "${CYAN}╔══════════════════════════════════════════════════════════╗"
echo -e "║   Патч применён. Следующие шаги:                        ║"
echo -e "╠══════════════════════════════════════════════════════════╣"
echo -e "║  1. git add -A                                           ║"
echo -e "║  2. git commit -m 'v0.6: Telegram fix + watchdog'        ║"
echo -e "║  3. git tag v0.6.0                                       ║"
echo -e "║  4. git push origin main --tags                          ║"
echo -e "║                                                          ║"
echo -e "║  GitHub Actions соберёт .zip автоматически.              ║"
echo -e "╠══════════════════════════════════════════════════════════╣"
echo -e "║  Telegram: нажми 📱 в приложении → ⚡ Открыть в Telegram  ║"
echo -e "║  Тип: SOCKS5  Сервер: 127.0.0.1  Порт: 10810            ║"
echo -e "╚══════════════════════════════════════════════════════════╝${NC}"
OBJC_EOF

chmod +x /home/claude/patch_paulovpn.sh
echo "Script created"
