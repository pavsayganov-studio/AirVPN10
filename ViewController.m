...
- (void)loadView {
    ...
    titleLabel.font = [NSFont fontWithName:@"HelveticaNeue-Light" size:26]
                   ?: [NSFont systemFontOfSize:26];
    ...
    [fm createDirectoryAtURL:appSupport withIntermediateDirectories:YES
...
- (NSDictionary *)parseVlessLink:(NSString *)link {
    ...
    for (NSURLQueryItem *item in comp.queryItems) {
        ...
        if ([item.name isEqualToString:@"sni"])
            tls[@"server_name"] = item.value;
...
- (void)processRawText:(NSString *)text {
    ...
    if (decoded) {
        NSString *s = [[NSString alloc] initWithData:decoded
    ...
    if (parsed.count == 0) {
        self.statusLabel.stringValue = @"Серверы не найдены."; return;
    }
...
- (void)downloadConfig {
    ...
    if ([urlString hasPrefix:@"vless://"]) {
        [self processRawText:urlString]; return;
    }
    ...
    [[[NSURLSession sharedSession]
      ...
      completionHandler:^(NSData *data, NSURLResponse *r, NSError *err) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (err || !data) {
                self.statusLabel.stringValue = @"Ошибка сети!"; return;
            }
            NSString *subPath = [[self.configPath stringByDeletingLastPathComponent]
                                  stringByAppendingPathComponent:@"subscription.json"];
...
- (void)startCoreOnly {
    ...
    // Outbounds: только выбранный сервер + direct + dns-out
    // Никакого urltest — ноль лишних соединений
    NSArray *outbounds = @[
        activeOutbound,
    ...
    // DNS: UDP 8.8.8.8 через direct, без DoH, без петель
    ...
    // Трафик к самому VPN-серверу — direct (защита от петли маршрута)
    ...
    if (self.stealthCheckbox.state == NSControlStateValueOn) {
        ...
        routeConfig = @{
            @"rules": rules, @"final": @"direct", @"auto_detect_interface": @YES
        };
    } else {
        routeConfig = @{
            @"rules": rules, @"final": activeTag, @"auto_detect_interface": @YES
        };
    }
    ...
    [[NSFileManager defaultManager] createFileAtPath:self.logPath
...
- (void)startVPN {
    ...
    [[[NSAppleScript alloc] initWithSource:[NSString stringWithFormat:
